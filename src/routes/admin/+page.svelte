<script lang="ts">
	...
	function handleRefresh() {
		loadVisitors();
		toast.info('Admin visitor logbook refreshed.');
	}
	...
	<header class="border-b border-border bg-card sticky top-0 z-40 shadow-xs">
		<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
			<div class="flex items-center gap-2 font-extrabold text-lg tracking-tight text-foreground">
				...
				<span>Calapexis Admin <span class="text-xs font-normal text-muted-foreground hidden sm:inline">| Security & Visitor Log Management</span></span>
			</div>
	...
	<main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
		...
		<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
			...
			<div class="flex items-center gap-2">
				...
				>
					<RefreshCwIcon class="size-3.5 text-primary" />
